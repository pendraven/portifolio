...
const TVScreen = ({ isOn, onOff }) => {
  ...
  if (powerState === 'off') {
    return <div style={{ width: '100%', height: '100%', background: '#111', borderRadius: 'inherit' }} />;
  }

  if (powerState === 'turning-on') {
    return <div style={{ width: '100%', height: '100%', background: '#000', animation: 'glitch 1s' }} />;
  }
  ...
  return (
    <div style={{ 
      ...
      <button 
        onClick={(e) => { e.stopPropagation(); onOff(); }} 
        style={{ position: 'absolute', top: 10, right: 10, zIndex: 20, cursor: 'pointer' }}
      >
        Desligar
      </button>
      
      {/* Renderização do Portfólio ou PDF */}
      {config.portfolioMode === 'pdf' && config.pdfUrl ? (
